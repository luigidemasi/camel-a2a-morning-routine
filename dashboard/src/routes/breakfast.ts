...
router.get('/api/breakfast-stream', async (_req, res) => {
  ...
  try {
    const client = await getClient(AGENTS.breakfast);
    const message = Message.fromJSON({
      messageId: crypto.randomUUID(),
      role: 'ROLE_USER',
      parts: [{ text: 'Make my breakfast' }],
    });
    ...
    console.error('Breakfast stream error:', err.message);
